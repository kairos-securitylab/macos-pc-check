...
# 알려진 백신/EDR 프로세스 및 앱 탐지 패턴
AV_PATTERN="AhnLab|V3|Falcon|CrowdStrike|Defender|Microsoft Defender|Sophos|Sentinel|SentinelOne|Carbon Black|Trend Micro|ESET|Bitdefender|Symantec|Norton|McAfee|Kaspersky|Malwarebytes"
...
if [ -z "$MIN_LEN" ]; then
    MIN_LEN="$(echo "$ACCOUNT_POLICY" | grep -Eo "matches[^{]*\{[0-9]+" | grep -Eo '[0-9]+' | head -n1)"
fi

# 복잡성 요구 여부: requires* 플래그(=1) 또는 정책 정규식(policyAttributePassword matches) 존재
COMPLEXITY=0
echo "$POLICY_ALL" | grep -Eqi 'requiresAlpha=1|requiresNumeric=1|requiresSymbol=1|requiresMixedCase=1|policyAttributePassword[^<]*matches' && COMPLEXITY=1
CLASS_CNT=0
for k in requiresAlpha requiresNumeric requiresSymbol requiresMixedCase; do
    echo "$POLICY_ALL" | grep -Eqi "${k}=1" && CLASS_CNT=$((CLASS_CNT+1))
done

echo ""
echo "2. 복잡성 판정 근거"
echo "최소 길이(minChars/minimumLength): ${MIN_LEN:-미확인}"
echo "복잡성 요구: $([ "$COMPLEXITY" -eq 1 ] && echo '있음' || echo '없음/미확인') (문자종류 요구 플래그 ${CLASS_CNT}종)"
echo "기준: 2종 이상 조합+10자 이상 또는 3종 이상 조합+8자 이상 (최소 ${PASSWORD_MIN_LENGTH}자)"
...
if [ -z "$GLOBAL_POLICY" ] && [ -z "$ACCOUNT_POLICY" ]; then
...
elif { [ -n "$MIN_LEN" ] && [ "$MIN_LEN" -ge 10 ] 2>/dev/null && [ "$CLASS_CNT" -ge 2 ]; } || \
     { [ -n "$MIN_LEN" ] && [ "$MIN_LEN" -ge "$PASSWORD_MIN_LENGTH" ] 2>/dev/null && [ "$COMPLEXITY" -eq 1 ]; }; then
    ...
    print_result "PC-02" "MANUAL" "복잡성 정책 충족 여부를 자동으로 확정하기 어려움(길이=${MIN_LEN:-미확인}, 복잡성=$([ "$COMPLEXITY" -eq 1 ] && echo '있음' || echo '미확인')). MDM 프로파일 또는 pwpolicy -getaccountpolicies 결과로 복잡성 정책 수동 확인 필요."
fi
end_check "2"
...
echo "[현황]"
echo "1. 프로세스 확인"
AV_PS="$(ps ax 2>/dev/null | grep -Eai "$AV_PATTERN" | grep -v grep | head -n 20)"
if [ -n "$AV_PS" ]; then
    echo "$AV_PS"
else
...
echo ""
echo "2. 응용프로그램 확인"
AV_APPS="$(find /Applications -maxdepth 2 -iname '*AhnLab*' -o -iname '*V3*' -o -iname '*Defender*' -o -iname '*Falcon*' -o -iname '*Sophos*' -o -iname '*Sentinel*' -o -iname '*ESET*' -o -iname '*Bitdefender*' -o -iname '*Malwarebytes*' 2>/dev/null | head -n 20)"
if [ -n "$AV_APPS" ]; then
    echo "$AV_APPS"
else
    echo "알려진 백신/EDR 응용프로그램 확인되지 않음"
fi
if [ -n "$AV_PS" ] || [ -n "$AV_APPS" ]; then
    print_result "PC-13" "GOOD" "백신/EDR 설치 또는 실행 상태가 확인됨. 업데이트 일자는 제품 관리 콘솔에서 추가 확인 권장."
else
    print_result "PC-13" "MANUAL" "스크립트로 알려진 백신/EDR 설치 여부를 확인하지 못함. 백신 또는 EDR 설치 및 업데이트 상태 증적 확인 필요."
fi
end_check "13"
